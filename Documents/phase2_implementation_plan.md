...
| 3 | PBF → `.odrg` 抽出ツール `OsmDotRoute.Extractor` CLI 実装（`System.CommandLine` ベース、PBF 読込 → 道路 way フィルタ → 頂点正規化 → エッジ生成 → AABB 計算 → STR R-tree 構築 → エッジフラグ bake → bake プロファイル → `.odrg` 書出） | REQ-MAP-008 | **進行中** — 3.1〜3.6 完了（CLI 雛形 + way フィルタ + 頂点正規化 + エッジ生成 + AABB + フラグ bake + プロファイル bake、115 テスト）、3.7〜3.9 未着手 |
...
- [x] ステップ 3 サブステップ分割 + 4-opt 判断ペンディング合意（2026-05-25、3.1〜3.9 + ステップ 3 完了後に 4-opt 再判断）
