...
| 2 | 独自 OSM PBF パーサー `OsmDotRoute.Pbf` 実装（protobuf ワイヤ形式 / varint / DenseNodes / Way / Relation / stringtable / ZLib 解凍、System.\* 完結） | REQ-MAP-007, REQ-DEP-002 | **着手中** — 2.1 プロジェクト骨格 ✅ / 2.2 ProtoReader ✅ (27 テスト) / 2.3 Blob + ZLib ✅ (25 テスト) / 2.4 HeaderBlock ✅ (18 テスト) / 2.5 PrimitiveBlock ✅ (19 テスト) / 2.6 単体 Node ✅ (15 テスト) / 2.7 DenseNodes ✅ (17 テスト) / 2.8 Way ✅ (16 テスト、id int64 plain varint + refs delta zigzag、PrimitiveBlock 不要) / 2.9〜2.10 未着手 |
...
- [ ] ステップ 2.9 着手（Relation 解析：members、tags。`type=restriction` は Phase 4+ 延期）
