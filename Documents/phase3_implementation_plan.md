...
| 指標 | Phase 1 値（津島市） | Phase 3 目標 | 検証ステップ |
| --- | --- | --- | --- |
| 経路計算（C0、89 ペア平均） | 33 ms | ≦ 33 ms | 3E |
| Itinero 比 | 0.48x | ≦ 0.48x（同等以下） | 3E |
| StdDev | Itinero の 1/7 | 同等維持 | 3E |
| 制約 100 件下（C1） | 51 ms（C0 比 1.43x） | ≦ 1.1× C0 | 3E |
| 経路 1 本あたりアロケート（C3） | 77 MB（Itinero 2.4x） | ≦ 5 MB（Phase 1 の 1/15、Itinero の 1/10） | 3E |
| 経路距離同等性（親プロ統合） | 89/89 ペア ±10% 以内、Mean 0.07% | 同等維持 | 3F |
| 都道府県単位（愛知県全域、C5） | 未測定（Phase 1 §18.2 未解消） | 実測値を取得（基準値は Phase 3 で確定） | 3G |
| 定常 WorkingSet | 54 MB（16 GB の 0.3%） | ≦ 54 MB（同等以下） | 3E |
