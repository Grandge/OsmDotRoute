...
- **【TODO】ユーザー定義プロファイル JSON の抽出ツール対応（将来 (B) で実装、REQ-PRF-009 関連、2026-05-29 ユーザー要望）**:
  コアは任意プロファイルに対応済み（`VehicleProfile.LoadFromJsonFile`、`ExtractPipeline` はプロファイル非依存で `profile.Name` 基準に bake、`.odrg` は名前一致でルーティング可能）だが、プロファイル名→定義の解決が組込み 4 種（car/pedestrian/bicycle/truck）にハードコードされており、ユーザー定義プロファイル（例: 緊急車両）の `.odrg` を抽出できない。ハードコード箇所は 3 つ:
  - CLI: [`Program.cs`](../src/OsmDotRoute.Extractor/Program.cs) `ResolveProfile`（switch、未対応名は例外）
  - Sandbox サーバ: [`ExtractEndpoints.cs`](../samples/Sandbox/Server/Endpoints/ExtractEndpoints.cs) `ResolveProfile`（switch、未対応名は 400）
  - Sandbox UI: [`ExtractPanel.tsx`](../samples/Sandbox/Web/src/components/ExtractPanel.tsx) `PROFILES` 固定配列
  - **(B) 本実装**: Sandbox UI で「カスタムプロファイル JSON 追加（ファイル選択/パス指定）」+ サーバ `ResolveProfile` 拡張 + チェックボックス動的化。
  - **(A) 最小先行案**（必要時）: CLI `--profiles` に `.json` パス指定を許可（`ResolveProfile` を「`.json` 実在パスなら `LoadFromJsonFile`、それ以外は組込み名」に拡張）。
  - 現状（2026-05-29）は (C) = 回答のみで未実装。緊急車両プロファイル自体の同梱は REQ-PRF-005/006（上記）として別途判断。
...
| 都道府県単位 愛知県（C5） | 未測定（Phase 1 §18.2） | **117 ms/route**（988k 頂点 / 1.4M エッジ） | 実測値を取得 | ✅ 取得済 | 3G |
| 都道府県単位 東京都（C5-T） | 未測定 | **288 ms/route**（1.28M 頂点 / 1.78M エッジ） | 実測値を取得 | ✅ 取得済 | 3G |
