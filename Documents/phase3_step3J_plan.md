...
**ステータス**: v0.2（**3J 全完了**、2026-05-29。3J.1〜3J.6 実装・検証済、`dotnet test` 682 件 pass）
...
| 0.2 | 2026-05-29 | **3J 全完了**。3J.1〜3J.6 実装・検証済。着手時判断確定: J-1=IL インタプリタ / J-2a=オーバーロード / J-3=津島市のみ / J-4a=Vite mode 別 alias / J-5a=単一静的サイト。実装中に判明した技術問題 2 件を解決（`JSType.MemoryView` 入力不可 → `byte[]` バルクマーシャリング、WASM のリフレクション JSON 無効 → プロファイル JSON ソース生成化）。コア追加: `RouterDb.LoadFromOdrg(ReadOnlyMemory<byte>)` / `GetProfileNames()` / `ProfileJsonContext`。`dotnet test` 682 件 pass。設計書 §10.7 肉付け（§0.3 表 10.5 → 10.7 改題）。gh-pages 実公開は J-C により 3H へ（ワークフロー deploy は `if: false`） | Claude (Opus 4.7) |
