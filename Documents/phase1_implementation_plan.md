...
### ステップ 11: 経路 → GeoJSON 出力ユーティリティ

**目的**: `Route` を GeoJSON LineString に変換するユーティリティを提供。

**作業**:
- `GeoJsonWriter.WriteRoute(Route) → string`（または独自 LineString オブジェクト）
- 経路シェイプを座標配列に変換、properties に総距離・総所要時間を含める

**完了判定**:
- 単体テスト: LineString 出力が GeoJSON 標準ビューア（geojson.io 等）で正しく描画される構文
- 設計書 §13「経路 GeoJSON 出力」を更新
