...
**ステータス**: サブステップ 3.1〜3.4 完了 / 3.5〜3.9 未着手
...
### 5.5 サブステップ 3.5 以降の予定

3.5 ではエッジ AABB（座標解決後）と `EdgeFlags`（仕様書 §4.5 の 14 bit）を bake する。エッジ AABB は座標が必要なため、3 パス目の座標ロード後に計算する。エッジフラグは tag のみ参照で bake 可能なため 3.4 直後でも実装可能。3.5 では両方を並行実装する見込み。
