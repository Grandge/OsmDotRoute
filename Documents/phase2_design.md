...
**ステータス**: サブステップ 3.1〜3.8 完了 / 3.9 未着手
...
### 5.9 サブステップ 3.9: 津島市 PBF 統合テストの予定

3.9 で実 PBF からの抽出パイプライン全体を組み立てる:

- PBF を 3 パス走査（仕様書 §5.3.3 アルゴリズム）
  - Pass 1: bbox 内ノード ID をビットセットに記録
  - Pass 2: 道路 way フィルタ → 必要ノード集合
  - Pass 3: 必要ノード座標をメモリ展開
- VertexNormalizer (3.3) → VertexAssignment
- EdgeGenerator (3.4) で全 way をエッジ列に
- 各エッジに対し AABB (3.5) / Flags (3.5) / Profile (3.6) を bake
- StrRTreeBuilder (3.7) で R-tree 構築 + EdgePermutation 取得
- 全エッジテーブルに permutation 適用
- OdrgWriter (3.8) で `.odrg` 書出
- Phase 1 RouterDb との頂点数 / 辺数 / 経緯度範囲突合
- 重要決定: 3.9 では「実 PBF を流す統合テスト」と「`Pipeline/ExtractPipeline.cs` を実装する設計」を両立する
