...
**ステータス**: サブステップ 3.1〜3.3 完了 / 3.4〜3.9 未着手
...
### 5.4 サブステップ 3.4 以降の予定

3.4 では各 way を `VertexAssignment` を使って頂点間のエッジに分割する。`fromVertexId` / `toVertexId` を採番し、頂点間に挟まれた中間ノード列を `shape` として保持。oneway / oneway=-1 / junction=roundabout（暗黙 oneway） から正逆向きを抽出してエッジを生成する。
