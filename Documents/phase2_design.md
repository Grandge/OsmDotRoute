...
**ステータス**: サブステップ 3.1〜3.7 完了 / 3.8〜3.9 未着手
...
### 5.8 サブステップ 3.8 以降の予定

3.8 で `.odrg` バイナリ書出を実装する。仕様書 §1〜§4 のレイアウトに従い、ヘッダー 256B → セクションテーブル → 各セクション本体 を順番に書く。

- セクション順は §1.1 のとおり: Vertex / Edge / Shape Buffer / AABB / Flag / Spatial Index (R-tree) / Baked Profile / Turn Restriction (length=0 予約) / Metadata
- リトルエンディアン固定、`MemoryMappedFile` 想定の連続バイト配置
- ヘッダー / セクションテーブルは最後にオフセット確定後上書き
- 3.7 の `EdgePermutation` を適用したエッジテーブル 4 種（EdgeRecord / Aabb / EdgeFlags / BakedProfileTable）を書く
- Metadata は UTF-8 JSON（生成日時 / 元 PBF パス / プロファイル名 / エッジフラグ bit 表 等）
