...
**ステータス**: サブステップ 3.1〜3.6 完了 / 3.7〜3.9 未着手
...
### 5.7 サブステップ 3.7 以降の予定

3.7 で STR (Sort-Tile-Recursive) パック静的 R-tree を構築する。各エッジ AABB の中心点を計算 → 経度ソート → ストリップ分割 → 各ストリップ内で緯度ソート → M ずつ葉ノード化 → 再帰的に内部ノード構築。仕様書 §4.6.1 のアルゴリズムに従い、§4.6.2 のレイアウト（1 ノード 56 byte、配列インデックス参照）でシリアライズ対応する。M=16 を初期値とする。
