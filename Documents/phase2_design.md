...
**ステータス**: サブステップ 3.1 完了（CLI 雛形）/ 3.2〜3.9 未着手
...
### 5.2 サブステップ 3.2 以降の予定

3.2 から実際の抽出パイプラインに着手。CLI 層が確定したため、`onExtract` ハンドラに渡される `ExtractOptions` を起点に、§5 冒頭リストの 9 段階を順次実装する。詳細は各サブステップ完了時に追加する。
