...
| 3. 独自バイナリグラフ形式 `.odrg` | ステップ 1 | 仕様書 v0.1 起こし済（[`phase2_graph_format_spec.md`](phase2_graph_format_spec.md)、本書 §3 はステップ 1 v0.2 確定時に「採用設計・判断根拠・トレードオフ」を抜粋記述） |
...
**ステータス**: 未記述（`phase2_graph_format_spec.md` 起こし時に初版、本章は同文書の要約 + 採用判断記録）

**初版時に書くこと**：

- 全体レイアウト図（ヘッダー / 頂点表 / エッジ表 / エッジ AABB 表 / エッジシェイプ連続バッファ / エッジフラグ / R-tree 索引 / bake プロファイル表 / メタデータ）
- セクション毎のバイナリレイアウト詳細（フィールド・バイトオフセット・型）
- マジックナンバー・バージョン番号体系・エンディアン
- STR パック R-tree のビルドアルゴリズム（変換時）・クエリアルゴリズム（Phase 3 ランタイム時）
- MMF レイアウト設計と `ReadOnlySpan<T>` ビュー切り出しパターン
- エッジフラグ bitflag 配置表（採用属性ごとのビット位置・1〜2 バイト確定理由）
- 採用しなかった案：uniform grid（密度ムラ非適応）、kd-tree（AABB クエリ非適合）、エッジ AABB float（精度不足）
- **Itinero RouterDb 構造に引きずられない自由設計の根拠**（ユーザー判断 2026-05-20）
- ファイルサイズ概算（津島市・愛知県・Phase 3 都道府県単位の見積もり）
...
**ステータス**: ステップ 2.1〜2.2 完了。2.3 以降は未着手。
...
### 4.2 ステップ 2.3 以降の予定（未着手）

- 2.3 PBF Blob 構造（`BlobHeader` 4 バイト BE サイズ + `BlobHeader` protobuf + `Blob` protobuf、ZLib 解凍）
