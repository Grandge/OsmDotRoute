...
**実装日**: 2026-05-28
**実装バージョン**: commit `bbe3af1`（3I.1-3）/ `793050e`（3I.4）/ `bc3c38f`（3I.5）
**主要ファイル**: `samples/Sandbox/Server/`（ASP.NET Core minimal API, port 5280）+ `samples/Sandbox/Web/`（React + Vite + MapLibre GL, port 5174）
...
`samples/Sandbox/Server`（`Microsoft.NET.Sdk.Web`、`OsmDotRoute` + `OsmDotRoute.Extractor` 参照）+ `samples/Sandbox/Web`（Vite + React + TypeScript + maplibre-gl）。Kestrel を `127.0.0.1:5280` 固定バインド、CORS は `localhost:5174` のみ。Web の vite proxy で `/api` → `127.0.0.1:5280`。MapVerifier の MapView.tsx / styles.ts / client.ts を流用。

#### 10.2.2 サブステップ 3I.2: PBF ダウンロード + bbox 範囲指定 UI

- `GeofabrikService`: 日本の**地方ブロック単位**（北海道/東北/関東/中部/近畿/中国/四国/九州 + 日本全国）の URL 定義。Geofabrik は都道府県別 extract を提供しないため地方ブロック + bbox 切出運用（計画書記載の「都道府県別」から修正）。User-Agent 明示、`HttpClient.Timeout = Infinite`（大容量 DL のタイムアウト回避）。
- `POST /api/download`: SSE ストリームで進捗 push（`text/event-stream`、インライン書込。当初の `Progress<T>` + 同期 `.GetAwaiter().GetResult()` がデッドロックを起こしたためインライン化）。
- `CacheService`: `%LOCALAPPDATA%/OsmDotRoute.Sandbox/cache`。保存先は `settings.json` に永続化（再起動後も維持、ユーザー要望）。
- bbox: MapView 上の 2 クリック矩形描画 + 描画後の 4 隅マーカードラッグ編集（MapLibre Marker `draggable`、ユーザー要望で親プロ Map&シナリオツール同等の編集性を実現）。
- `FileBrowserDialog`: 保存先フォルダ参照 + 既存 PBF ブラウズ（MapVerifier 流用、`folderMode` 追加）。

#### 10.2.3 サブステップ 3I.3: `.odrg` 抽出パイプライン統合

- §5.5-31 確定（同プロセス内呼出）。`ExtractEndpoints` が `ExtractPipeline.Run()` を `Task.Run` で実行（IProgress 非対応のためフェーズレベル進捗：extracting → writing → loading → complete を SSE 送出）。`Sandbox.Server` を `OsmDotRoute.Extractor` の `InternalsVisibleTo` に追加。
- 抽出後 `RouterDb.LoadFromOdrg` で自動ロード → `GET /api/road-network` で道路 NW を GeoJSON 表示。
- `LoadEndpoints`（既存 .odrg ロード）も提供。プロファイル名は `OdrgReader` で読出。

#### 10.2.4 サブステップ 3I.4: ルート探査 UI

- `RouteEndpoints`: `POST /api/snap`（座標スナップ）+ `POST /api/route`（経路計算 → `GeoJsonConverter` で LineString）。
- `RoutePanel`: From/To ピックモード（マップクリック）、プロファイル選択（ロード済 .odrg の bake プロファイルのみ）、Route/Re-Route/Clear。
- プロファイル反映: ロード/抽出時に対象プロファイルを取得し ExtractPanel チェックボックス + RoutePanel ドロップダウンに反映。

#### 10.2.5 サブステップ 3I.5: メッシュ / ポリゴン制約付与

- `MeshEndpoints`: `GET /api/mesh/grid`（`MeshCode.EnumerateInBounds` で 1km/500m/250m グリッド GeoJSON、10,000 セル上限ガード）。ステートレス。
- `RestrictionEndpoints`: polygon/mesh 制約の登録・一覧・GeoJSON・削除・全クリア。`SandboxState.Restrictions`（`RestrictedAreaService`）を使用。
- `SandboxState` に `RestrictedAreaService` を統合。Load/Extract 時に `new RestrictedAreaService()` を生成し `new Router(routerDb, restrictions)` で注入（コンストラクタが `AttachGraph` 自動実行）。新グラフロード毎に新規生成 = **セッション単位**（前グラフの制約を持ち越さない、Sandbox 向け方針。MapVerifier の DI シングルトン共有とは異なる）。
- **Re-Route**: `Router.Calculate` が内部で `_restrictions`（= `EdgeWeightCalculator` 経由）を評価するため、制約登録後の Re-Route で block 回避 / difficulty 速度低下が即時反映。3I キラー機能。
- Web: MeshGridPanel / PolygonEditorPanel / RestrictionListPanel を MapVerifier から流用。App.tsx でメッシュクリック選択・ポリゴン頂点追加・制約一覧 refresh を統合。

### 10.3 設計判断の根拠

- **React + MapLibre GL（計画書の Leaflet から変更）**: MapVerifier 資産（MapView / 各パネル / client.ts）の再利用で開発効率最大化。
- **Geofabrik 地方ブロック単位（都道府県別から修正）**: Geofabrik が日本の都道府県別 extract を提供しないため。bbox 切出で都道府県相当を実現。
- **SSE インライン書込**: `Progress<T>` コールバック内の同期待機がデッドロックを誘発したため、ダウンロード/抽出ループ内で直接 SSE 書込。
- **制約のセッション単位リセット**: ユーザー試用デモでは「ロードした .odrg に対して制約を試す」が自然。前グラフの制約持ち越しは混乱を招くため新グラフ毎にリセット。
...
- `dotnet test` **676 件 pass** 維持（Sandbox はサンプルのため単体テスト対象外、ライブラリ側回帰がないことを確認）。
- 手動検証: `start-sandbox.ps1` で起動 → PBF DL → bbox 描画/ドラッグ → 抽出 → 道路 NW 表示 → From/To 経路 → メッシュ/ポリゴン制約登録 → Re-Route で回避経路確認。各サブステップ完了時にユーザー目視確認済。

### 10.6 実装メモ

- `start-sandbox.ps1`: Server + Web を別ウィンドウで起動（MapVerifier の `start-map-verifier.ps1` と同構成）。
- bbox 表示は `.odrg` v0.3 の `bboxRequested`（要求 bbox）を優先（§? Phase 2 仕様書 v0.3、way 拡張で実データ範囲が要求 bbox を超える問題に対応）。旧 v0.2 は頂点 AABB にフォールバック。
- MapView の click ハンドラ優先順位: ① bbox 描画 → ② ポリゴン描画 → ③ ルートピック → ④ メッシュ選択。マーカードラッグ中は click を抑止。

---

## 10.7 GitHub Pages デモ（React 流用 + コア WASM 化）

**対応ステップ**: 3J（6 サブステップ 3J.1〜3J.6）
**対応要件**: REQ-PKG-003（OSS 公開準備の一環）、REQ-LIC-004、REQ-MAP-005（`.odrg` ランタイム読込のブラウザ拡張）
**実装日**: 2026-05-29
**実装バージョン**: `d4cac88`（3J.1）/ `d8b8e2d`（3J.2）/ `a22323d`（3J.3）/ `9822e82`（3J.4）/ `c49c103`（3J.5）/ 本コミット（3J.6）
**主要ファイル**: `samples/Sandbox.Wasm/`（WebAssembly Browser App + JSExport ブリッジ）+ `samples/Sandbox/Web/src/api/wasmClient.ts` ほか + `.github/workflows/sandbox-wasm-pages.yml`
**計画書**: [`phase3_step3J_plan.md`](phase3_step3J_plan.md)（v0.1）

### 10.7.1 意図

3I の Sandbox はローカル実行前提（ASP.NET Core サーバ + React）。本ステップは OSS 公開時に「インストール不要で即体験」できるブラウザデモを GitHub Pages 用静的成果物として用意する。Sandbox のサブセット（経路計算・メッシュ/ポリゴン制約・Re-Route）を、**OsmDotRoute コアエンジンを WebAssembly 化**してブラウザ内で実行し、UI は 3I の React 資産を流用する。PBF ダウンロード / 抽出は CORS・処理量の制約から対象外（事前ビルド `.odrg` 同梱で代替）。

**原計画 §3.9 からの修正**: 実装計画書 §3.9 は「Sandbox の Web コンポーネントを Blazor interop で流用」と記述していたが、3I の Web は **React + MapLibre GL** であり Blazor ではない。ユーザー確認（2026-05-29）で以下 3 方針に確定し再定義した。

| # | 判断 | 確定 |
| --- | --- | --- |
| J-A | UI 方式 | **React 流用**（WASM 化はコアエンジンのみ） |
| J-B | コア改修 | **in-memory `.odrg` ソースをコアに追加** |
| J-C | gh-pages 実公開時期 | **3H と同時**（3J はビルド + CI 整備まで） |

### 10.7.2 採用設計（サブステップ別）

- **3J.1 WASM 雛形**: `samples/Sandbox.Wasm`（`Microsoft.NET.Sdk.WebAssembly`、net9.0、`wasm-tools` ワークロード）。コアをプロジェクト参照。J-1 = IL インタプリタ既定（AOT は性能不足が判明したら検討）。初回 DL は brotli 約 1.9 MB（ネイティブランタイム 0.9 MB + マネージド、コア本体は 14.6 KB）。
- **3J.2 コア in-memory ソース**: `OdrgMmfHandle` を MMF / メモリ両対応の単一 sealed クラス化（`CreateFromMemory(ReadOnlyMemory<byte>)` 追加、`ReadOnlyMemory.Pin()` でピン留め）。ホットパスの `GetSpan`/`GetRawSpan` は**インターフェース化せず直接呼出のまま**として性能退行を回避（§3 NativeRoadGraph の補足）。`NativeRoadGraph(ReadOnlyMemory<byte>)` ctor + `RouterDb.LoadFromOdrg(ReadOnlyMemory<byte>)` 非破壊オーバーロード（J-2a）。MMF 固有の `ViewHandle` 依存を `OdrgSectionDirectory.Parse(GetRawSpan(...))` に置換。MMF/in-memory 完全一致の突合テスト 6 件追加。
- **3J.3 ルーティングブリッジ**: `Sandbox.Wasm.Interop`（JSExport）に `LoadOdrg(byte[])` / `GetStats` / `GetRoadNetwork` / `CalculateRoute` / `Snap` を実装。React 側 `client.ts` の DTO 形状に一致する JSON を返す。事前ビルド `.odrg`（J-3 = 津島市のみ、8 MB）をビルド時に `wwwroot/data` へコピー同梱。
- **3J.4 制約 + メッシュブリッジ**: `AddPolygonRestriction` / `AddMeshRestriction`（block / difficulty）/ `ListRestrictions` / `RestrictionsGeoJson` / `DeleteRestriction` / `ClearRestrictions` / `MeshGrid`。`LoadOdrg` で `RestrictedAreaService` を `Router` に attach するため、制約付与後に `CalculateRoute` を再呼出すれば Re-Route（回避）。GeoJSON は `Utf8JsonWriter` で生成。
- **3J.5 React 統合**: Vite の mode 別 `resolve.alias`（`--mode wasm` で `../api/client` → `wasmClient.ts`）により**全コンポーネント無改変**で API バックエンドを差替（J-4a）。`wasmClient.ts` は `client.ts` と同一インターフェース（PBF/抽出/キャッシュ系は WASM 非対応スタブ）。`App.tsx` は `import.meta.env.MODE==='wasm'` で DownloadPanel/ExtractPanel を隠し `PresetPanel`（事前ビルド `.odrg` プルダウン）を表示。`scripts/prepare-wasm.mjs` + `npm run build:wasm` で WASM publish → React `public/` へ取込み、React + WASM + `.odrg` を単一静的サイト化（J-5a）。
- **3J.6 CI 整備**: `.github/workflows/sandbox-wasm-pages.yml`（.NET 9 + `wasm-tools` + Node 20 → `npm run build:wasm` → Pages artifact）。J-C により deploy ジョブは `if: false` で無効化（build/artifact 検証まで、実公開は 3H）。

### 10.7.3 設計判断の根拠 / 解決した技術問題

- **インターフェース化の回避（性能）**: データソースを `IOdrgDataSource` 的に抽象化すると `GetSpan` 呼出が仮想ディスパッチになり、Phase 3 §9 の C0（7.70 ms）等が退行する恐れ。単一 sealed クラスで MMF / メモリ両モードを内部分岐し、ホットパスは直接呼出を維持した。
- **JS → .NET バイナリ受け渡し**: `[JSMarshalAs<JSType.MemoryView>]` は .NET メモリを JS へ公開する用途で、JS データの入力には不適（実行時 `Expected MemoryViewType.Byte`）。`byte[]` 既定バルクマーシャリング（JS `Uint8Array` → マネージド配列）に変更。
- **WASM のリフレクション JSON 無効化**: WASM 既定で `JsonSerializerIsReflectionEnabledByDefault=false` のため、コアの `VehicleProfile.LoadFromJsonStream`（リフレクション STJ）が実行時例外。プロファイル JSON 読込を**ソース生成**（`ProfileJsonContext`）に変更し、コア全体を trim/AOT 安全化（IL2026 警告も解消）。
- **プロファイル名公開**: Server は Extractor の `OdrgReader` でベイク済プロファイル名を取得していたが、WASM は Extractor を参照しないため `RouterDb.GetProfileNames()` を新設。

### 10.7.4 トレードオフ・制約

- PBF ダウンロード / 抽出は WASM 非対応（事前ビルド `.odrg` 同梱で代替）。React 側の該当パネルは WASM モードで非表示。
- 同梱 `.odrg` は津島市のみ（J-3）。aichi/tokyo は GitHub の 100 MB/ファイル上限超過のため不可。
- gh-pages 実公開は未実施（J-C、3H で REQ-PKG-002 解除と同時）。
- 制約はセッション中のみ（3I と同方針、リロードで消失）。

### 10.7.5 検証方法

- `dotnet test` **682 件 pass**（3J.2 で in-memory 突合 6 件追加。コア改修の回帰なし）。
- トリム済み WASM publish を Node ホストで起動し、`.odrg` ロード（53,727 頂点 / 4 プロファイル）・4 プロファイル全経路（profile 毎に距離差）・snap・道路 NW GeoJSON・ポリゴン block 付与での Re-Route 回避（blocked box 内経由点 39→0、距離 2172 m→2506 m 迂回）・メッシュグリッド（250 m で 88 セル）・一覧/GeoJSON/削除/クリアを確認。
- `npm run build:wasm` で単一静的サイト生成（`index.html` + `assets` + `_framework` + `data/tsushima.odrg`）。dist の `_framework` ランタイムを Node で起動しロード + 経路計算成功。非 WASM `npm run build`（server モード）も無回帰。
- **未検証**: React UI のブラウザ実描画（ヘッドレスブラウザ未使用）。依存する WASM ブリッジと静的サイト組み立ては検証済み。

### 10.7.6 実装メモ

- `wasmRuntime.ts`: `dotnet.js` / `.odrg` の URL は `import.meta.env.BASE_URL` 基準で解決（GitHub Pages サブパス対応）。`getInterop()` で遅延初期化。
- `vite.config.ts`: `defineConfig(({ mode }) => ...)`。`base` は `SANDBOX_BASE` 環境変数（CI が `/<repo>/` を設定）または `/`。`vite.config.js` / `.d.ts` は `tsc -b` 生成物で gitignore（Vite は `.js` を優先するため build スクリプトが毎回再生成）。
- ビルド成果物（`samples/Sandbox.Wasm/wwwroot/data/`、`samples/Sandbox/Web/public/_framework`・`data`、`dist/`）はすべて gitignore。実体 `.odrg` は git 追跡済の `samples/Data/tsushima.odrg` を流用。
...
- **制約 I/O（JSON 保存/読込）**: 読込は両モードともアップロード JSON を `registerMeshRestriction` / `registerPolygonRestriction` で 1 件ずつ再登録。保存はモード別 — **Server モードは「保存先フォルダ設定」（`CacheService.CacheDir`）へサーバー側で書込**（`POST /api/restrictions/save` → ライブラリ `SaveToJsonFile`、`restrictions-yyyyMMdd-HHmmss.json`）、**WASM モードはブラウザのダウンロード**（`saveRestrictions` を mode 別 alias で差替）。形式は `{ "format": "osmdotroute-restrictions", "version": 1, "items": [...] }`。
...
| — | 2026-06-01 | §10.8 追補。Server モードの制約保存先を「保存先フォルダ設定」（`CacheService.CacheDir`）に変更（`POST /api/restrictions/save` 新設、ライブラリ `SaveToJsonFile` 使用）。WASM モードは従来どおりブラウザダウンロード。Sandbox デモ版を 1.1.1 に更新 | Claude (Opus 4.8) |
| — | 2026-06-01 | サンプル整理。Phase 1 で作成後 "Hello World" 雛形のまま放置されていた `samples/ConsoleDemo`（OsmDotRoute 未使用）を削除（dir + sln 参照）。手動確認は Sandbox + 単体テストで充足 | Claude (Opus 4.8) |
