...
- ⏳ 本計画書 v0.1 のユーザー承認 → 承認後 commit → 3C.1 着手

**ユーザー承認後の進め方**: 3A / 3B / 3D と同様、各サブステップ着手前に必要に応じて事前調査と判断を実施し、計画書を v0.2 / v0.3 と更新する。サブステップ完了毎に commit し、3C.5 完了時に最終 commit で設計書 §6 を肉付けする。

**Phase 3 計画書 §3.4 文言との差分**:

- 「`MapService.LoadFromRouterDb` / `LoadFromOsmPbf`」→ 実態不在のため `RouterDb.LoadFromOdrg` 新設で対応（Q1 = A 確定）
- 「Phase 1 既存 526 件全 pass 維持」原則は 3C で意図的に解消（Q3 = A 確定、Itinero 経由テストは `.odrg` ベースに全書換）
- これらは Phase 3 計画書 §3.4 の意図と整合（Itinero 完全撤去 = 必然的に Itinero 系テスト書換）
